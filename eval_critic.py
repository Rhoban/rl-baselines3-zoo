...
def get_reset_dict_arr(situation: int, nb_tests: int = 1000, lr: bool = False, obstacle_radius = 0) -> np.ndarray:
    ...
    if situation == 1:
        ...
        dthetas = [0]
        dxys = [[0.0, 0.20], [0.0, 0.0], [0.0, -0.20]]
        size_list = len(dxys) * len(dthetas)
        size_list *= 2 if lr else 1
        reset_dict_arr = np.empty(shape=(0, size_list))
...
for situation in range(1, 4):
    ...
    for obs_radius in obstacle_radius:
        ...
        complete_more_steps_array = np.array([])
        ...
        for reset_dict_exp in tqdm(reset_dict_arr, disable=False):
            ...
            if index_min_total_step != index_min_critic:
                ...
                if more_steps != 0:
                    more_steps_array = np.append(more_steps_array, more_steps)
                    
            complete_more_steps_array = np.append(complete_more_steps_array, nb_steps_arr[index_min_critic] - nb_steps_arr[index_min_total_step])
        ...
        extra_steps_erroneous_choice_array = np.array([])
        ...
        print(f"Mean Difference Critic/Steps: {np.round(mean_diff_critic_steps,2)}, SD Difference Critic/Steps: {np.round(np.std(mean_diff_critic_steps),2)}")
